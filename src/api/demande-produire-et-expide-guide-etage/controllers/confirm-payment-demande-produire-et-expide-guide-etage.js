...
module.exports = {
  async confirmPayment(ctx) {
    const { sessionId, type_travail, caseNumber, patient,offre,Immediate_Loading,Metal_Impression_First_Stage,Resin_Impression_of_Both_Stages,Metal_Impression_of_Both_Stages,originalCost } = ctx.request.body;
    ...
    try {
      ...
      if (session.payment_status === "paid") { 
        ...
        // Create new commande
        const produide = await strapi.service("api::demande-produire-et-expide-guide-etage.demande-produire-et-expide-guide-etage").create({
          data: {
            case_number: caseNumber,
            type_travail: type_travail,
            Cost: commande.cost,
            Patient:patient,
            Immediate_Loading:Immediate_Loading,
            Metal_Impression_First_Stage:Metal_Impression_First_Stage,
            Resin_Impression_of_Both_Stages:Resin_Impression_of_Both_Stages,
            Metal_Impression_of_Both_Stages:Metal_Impression_of_Both_Stages
          },
        });
    ...
    <div style="max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
      ...
      <div style="padding: 20px 0;">
        ...
        <div style="padding: 20px 0; border-top: 2px solid #ffd700;">
            ...
            <p style="margin: 10px 0; color: #000;"><strong>Immediate Loading:</strong> ${
              Immediate_Loading ? checkIcon : xIcon
            }</p>
            <p style="margin: 10px 0; color: #000;"><strong>Resin Impression of Both Stages:</strong> ${
              Resin_Impression_of_Both_Stages ? checkIcon : xIcon
            }</p>
            <p style="margin: 10px 0; color: #000;"><strong>Metal Impression First Stage:</strong> ${
              Metal_Impression_First_Stage ? checkIcon : xIcon
            }</p>
            <p style="margin: 10px 0; color: #000;"><strong>Metal Impression of Both Stages:</strong> ${
              Metal_Impression_of_Both_Stages ? checkIcon : xIcon
            }</p>
        </div>`;
