  ...
  async confirmPayment(ctx) {
    ...
    try {
      ...
      if (session.payment_status === 'paid') {
        ...
        const guide = await strapi.db.query('api::guide-a-etage.guide-a-etage').findOne({
    ...
    <div style="max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
      ...
      <div style="padding: 20px 0;">
        ...
        const getOptionIcons = (options) => {
            return options.map(option => `${option.title}: ${option.active ? checkIcon : xIcon}`).join('<br>');
        };

        const getComponentIcons = (components) => {
            return components.map(component => `${component.active ? checkIcon : xIcon}`).join(' ');
        };
        ...
        <div style="padding: 20px 0; border-top: 2px solid #ffd700;">
            ...
            <p style="margin: 10px 0; color: #000;"><strong>Case Number:</strong> ${guide.numero_cas}</p>
            <p style="margin: 10px 0; color: #000;"><strong>Patient:</strong> ${guide.patient}</p>
            <p style="margin: 10px 0; color: #000;"><strong>Comment:</strong> ${guide.comment}</p>
            ...
            <h4 style="color: #000; margin-top: 20px; border-bottom: 1px solid #ddd; padding-bottom: 10px;">Full Guidee: ${getComponentIcons(guide.Full_guidee)}</h4>
            
            <h4 style="color: #000; margin-top: 20px; border-bottom: 1px solid #ddd; padding-bottom: 10px;">Forage Pilote: ${getComponentIcons(guide.Forage_pilote)}</h4>
            
            <h4 style="color: #000; margin-top: 20px; border-bottom: 1px solid #ddd; padding-bottom: 10px;">Marque de la Clavette: ${guide.Marque_de_la_clavette.map(mc => `${mc.description}`)}</h4>
            
            <h4 style="color: #000; margin-top: 20px; border-bottom: 1px solid #ddd; padding-bottom: 10px;">Marque de la Trousse: ${guide.Marque_de_la_trousse.map(mt => `${mt.description}`)}</h4>
            
            <h4 style="color: #000; margin-top: 20px; padding-bottom: 10px;">Marque Implant pour la Dent:</h4>
            <p style="margin: 10px 0; color: #000;">${Object.entries(guide.marque_implant_pour_la_dent[' index']).map(([key, value]) => `${key}: ${value}`).join('<br>')}</p>
        </div>`;
        }
